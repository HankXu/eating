...
export class OrderService {
    ...
    //编写获取数据的方法
    getOrderList(): Promise<object[]> {
        
        return this.http
                .get(this.getOrderListUrl)  //调用http内置的各个类型的获取方法，get/post/put/delete等
                .toPromise()  //http方法返回的是observable对象，所以将其转换为Promise对象，也可以不转换直接用Observable对象，看你自己需求，因为两个都要现学
                .then( response => response.json().data as object[])  //Promise技术的then方法，当请求成功时会进入这一个方法，调用你传入的处理方法，这里用了箭头表达式，不用这个也可以直接用普通的函数模式，
